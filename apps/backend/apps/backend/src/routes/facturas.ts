...
router.get('/', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if ((pageStr && (isNaN(page) || page < 1)) || (limitStr && (isNaN(limit) || limit < 1 || limit > 100))) {
      res.status(400).json({
        error: 'ValidationError',
        message: 'Parámetros inválidos. page >= 1, limit entre 1-100',
      });
      return;
    }

    // Parsear sort (default: fecha_factura:desc)
    ...
    if (!validSortFields.includes(sortField) || !validSortOrders.includes(sortOrder)) {
      res.status(400).json({
        error: 'ValidationError',
        message: `Sort inválido. Campos válidos: ${validSortFields.join(', ')}. Órdenes: asc, desc`,
      });
      return;
    }

    // Map custom field names a columnas reales
    ...
    // Obtener facturas con paginación y ordenamiento
...
// GET /:id, PUT /:id, DELETE /:id — Sprint 2 continuación
